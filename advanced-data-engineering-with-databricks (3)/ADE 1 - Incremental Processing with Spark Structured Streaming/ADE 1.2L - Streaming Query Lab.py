...
# TODO
df = (spark.readStream.load(DA.paths.sales)
# df = (spark.readStream.option("path", DA.paths.sales).load()
)
...
df2 = df.withColumn("items", explode(col("items"))).filter(col("items.coupon").isNotNull())

#                  .filter(col("items.coupon").isNotNull())

#count = df2.count()
#print("Number of rows in df2:", count)
df2.display()
...
DA.validate_3_1(coupon_sales_query1)
